...
| Ombre : réduit l'**initiation** des latéraux (pas seulement l'élagage *a posteriori*) | ✅ | `sim/shade_avoidance.py::lateral_break_probability` — à l'émission, chaque latéral ne débourre ACTIF qu'avec une probabilité `1 − strength·(1 − light_factor)` (lue sur le bourgeon-mère) ; sinon il démarre **RESERVE** (`dormant_reserve_buds`, réactivable par la réitération existante `activate_reserves_on_shed`). `sim.shade_avoidance.strength` ∈ [0,1] = fraction de latéraux retenus à l'ombre pleine. **Off par défaut** (`enabled=False`) / `strength=0` / plein soleil ⇒ aucun tirage RNG, évolution **byte-identique** (presets & goldens inchangés). Complète `shade_mortality` : celui-ci **retient** l'investissement à l'initiation, celui-là **élague** les survivants. Diagnostic `lateral_reserve_fraction` (lu sur le graphe de bourgeons → reflète le `.glb`). Latent dans les presets (prouvé par test d'intégration semé : chêne auto-ombragé, `dormant_reserve_count=0` + `shade_mortality` off pour isoler l'initiation). [#63](https://github.com/julien-riel/palubicki/issues/63) livré |
