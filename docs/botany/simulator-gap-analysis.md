...
| Whorled | 🟡 | defect found | `k=whorl_count` (pine: ×5 @ 72°). **No inter-whorl rotation** (`phyllotaxy.py:70-71,88-89`): successive whorls aren't offset, so pine's `divergence=72°=360/5` stacks every whorl onto the same 5 azimuths (5 vertical ranks) instead of interleaving | M | L | **FIX** ([#35](https://github.com/julien-riel/palubicki/issues/35)) — mirror the decussate half-step offset (`phyllotaxy.py:62-66`) |
...
**Verdict.** The cross-cutting per-axis delivery defect is **fixed** ([#24](https://github.com/julien-riel/palubicki/issues/24)): divergence advances per-axis via `Bud.axis_node_ordinal`, so spiral, decussate and distichous are all delivered correctly along each axis on the real tree (guarded by [#25](https://github.com/julien-riel/palubicki/issues/25)). With true distichous ([issue #15](https://github.com/julien-riel/palubicki/issues/15)) + `distichous_on_plagiotropic` giving flat 2-ranked conifer sprays, those modes read correctly. One defect remains, found in the 2026-05-29 eval: **`whorled` has no inter-whorl rotation** ([#35](https://github.com/julien-riel/palubicki/issues/35)), so pine's whorls stack into vertical ranks rather than interleaving — the fix mirrors the decussate half-step offset. The juvenile-vs-adult phyllotaxis switch stays **SKIP**.
