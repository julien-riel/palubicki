...
This doc reports the **current** status of each botanical concept against the
simulator. The Δ column records how a row moved relative to the v1 baseline
(BHse), kept as a quick read of where the model has been deepening; the history
of *when* each change landed is in git and in [`../roadmap.md`](../roadmap.md).
