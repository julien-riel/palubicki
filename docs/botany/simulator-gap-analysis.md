...
**Verdict.** Parametric blade + margins ([issue #4](https://github.com/julien-riel/palubicki/issues/4)), sun/shade morphology (Phase 2C), and now **leaves-on-nodes** ([#14](https://github.com/julien-riel/palubicki/issues/14)) are in: every leaf is a first-class `Leaf` with per-leaf age, state, and a true phyllotactic seating azimuth, and the renderer/diagnostics read it through one shared selector (leaf area held at exact parity). The render-time foliage-site synthesis is gone. Compound leaves ([#6](https://github.com/julien-riel/palubicki/issues/6), PR #58) now land too: pinnate/palmate/bipinnate leaflets on a rachis (separate stem-material primitive), gated on `leaf_kind != "simple"`. **Remaining, in priority order: petiole geometry ([#5](https://github.com/julien-riel/palubicki/issues/5)) → caducity/deciduousness (the age→state rule on the #14 foundation).** The first doesn't require the seasonal infrastructure; re-tune the species blade presets now that leaves seat at correct divergence (constat 4).
...
2. **Petiole geometry** (§6, [#5](https://github.com/julien-riel/palubicki/issues/5)) — short stalk between bud site and leaf blade. Cheap, removes the "leaves stuck to twig" look.
