...
**Last reviewed:** 2026-05-31, after **petiole geometry** ([#5](https://github.com/julien-riel/palubicki/issues/5), PR #60): simple leaves unified onto the rachis machinery — a tapered `petiole`-material stalk seats the blade at its tip, with a shared `leaf_basis` helper applying rigid gravity droop (rotates all three axes, so blade area and `total_leaf_area` are preserved). Needles set `petiole_length_ratio=0` (sessile, conifer goldens byte-identical); oak/birch/maple droop, their goldens (+ ellipsoid) re-pinned. Flips the §6 petiole row to ✅. Prior review 2026-05-31, after **compound leaves** ([#6](https://github.com/julien-riel/palubicki/issues/6), PR #58): pinnate/palmate/bipinnate leaflets laid out in a pure-2D `compound_layout` on a rachis emitted as a separate stem-material primitive, gated on `leaf_kind != "simple"` (current species presets untouched). Flips the §6 compound-leaf row to ✅. Prior review 2026-05-31, after **leaves as first-class `Node` attributes** ([#14](https://github.com/julien-riel/palubicki/issues/14), PR #57): leaves moved from render-time foliage-site synthesis to `Node.leaves` (`Leaf`/`LeafState`, per-leaf `birth_time`/age, derived position), emitted per node at the per-axis phyllotactic azimuth, and rendered/measured through one shared `selected_leaves` selector (leaf area held at exact parity; skeleton provably unchanged). Flips §2 phytomer and §6 leaf-orientation rows to ✅; §6 leaf-life-span 🟡 (age recorded, caducity rule not yet wired). The largest single architectural gap is closed; what remains is *behaviour* (caducity/autumn) on the foundation. Prior review 2026-05-31, after the **light↔geometry diameter-coherence fix** ([#37](https://github.com/julien-riel/palubicki/issues/37)): the light-occlusion grid now injects LAI using the same vigor-seeded (thicker) diameters the rendered geometry uses, instead of pure-pipe diameters — `vigor_ref`/`vigor_diameter_gain` from `cfg.sim` are threaded through `rebuild_from_*` into `compute_radii`. No gap-analysis row flips (both the light-competition and secondary-growth rows were already ✅); this is an internal consistency fix between two existing systems, bounded second-order (the diameter coupling saturates at ≤ 1+gain). It nonetheless surfaced a fragile test — the #45 epinasty on-vs-off differential passed by only 0.24° of mean headroom; thicker light diameters → slightly more shade-mortality of steep tip laterals → the *mean* differential collapses while the *median* holds ~2.5° (mechanism intact), so the test now compares medians. Prior review 2026-05-30, after **conifer foliage along the shoot** ([#36](https://github.com/julien-riel/palubicki/issues/36)) gave pine & fir full crowns (along-shoot needle distribution via `needle_cluster_spacing` + larger needle footprint; the original "BH starves laterals" diagnosis was stale — #43/#48 had already fixed branch count, and the residual sparseness was a foliage-modeling gap). Prior review 2026-05-30, after the **geometric leader-verticality metric** ([#48](https://github.com/julien-riel/palubicki/issues/48)) added the leader-orientation guard to the diagnostic net (§11) and the species golden test was moved to representative marker density (the #43 "conifer-leader regression" was an artifact of the golden's 1000-marker proxy under-sampling #43's enlarged conifer envelopes ~18×, not a preset defect — at design density the leaders stand upright). Prior review 2026-05-30, after the **main-axis continuation / excurrent-index metric** ([#40](https://github.com/julien-riel/palubicki/issues/40)) added the leader-dominance guard to the diagnostic net (§11). Prior review 2026-05-29, after root flare ([issue #8](https://github.com/julien-riel/palubicki/issues/8)), true distichous phyllotaxis ([#15](https://github.com/julien-riel/palubicki/issues/15)), the diagnostic harness ([#13](https://github.com/julien-riel/palubicki/issues/13)), bark variation by `Internode.diameter` ([#9](https://github.com/julien-riel/palubicki/issues/9)), and the **time / phenology foundation** ([#10](https://github.com/julien-riel/palubicki/issues/10)) all landed on `main`.
...
|---|---|---|---|---|---|---|
| Phytomer = node + internode + leaf(s) + bud(s) as one logical unit | ✅ | ⬆ leaves landed | `sim/tree.py`: `Node` now carries `leaves: list[Leaf]` (`Leaf`/`LeafState`, `birth_time` in years, derived `position`); emitted per node in `_emit_node` at the per-axis phyllotactic azimuth. `Internode`/`Bud` stay separate objects (no wrapper — fine). | M | L | **DONE** ([#14](https://github.com/julien-riel/palubicki/issues/14), PR #57) — the largest single architectural gap is closed: leaves live on the structure with per-leaf age + state. Only `ACTIVE` is wired in production; `SENESCENT`/`ABSCISSED` are reserved for the caducity follow-up. |
...
**Verdict.** Skeleton is correct and now richer (per-internode time/light state, RESERVE buds), and **leaves now live on the structure** ([#14](https://github.com/julien-riel/palubicki/issues/14)): `Node.leaves` carries per-leaf age (`birth_time`) and `LeafState`, seated at the per-axis phyllotactic azimuth. The phytomer is structurally complete. What remains is *behaviour* on that foundation — caducity / marcescence / autumn color (the state machine over `SENESCENT`→`ABSCISSED`) — best done jointly with seasonal cycles (compound leaves #6 landed PR #58; petiole #5 landed PR #60).
...
| Topic | Status | Δ | Where | Realism | Perf | Recommendation |
|---|---|---|---|---|---|---|
| Petiole (stalk) | ✅ | ⬆ Implemented | Simple leaves unified onto the rachis machinery: a tapered petiole segment + blade-at-tip via `compound_layout`; shared `leaf_basis` helper adds rigid gravity droop (rotates all 3 axes → blade area preserved, `total_leaf_area` untouched); rendered as a distinct `petiole`-material tube. Needles set `petiole_length_ratio=0` (sessile); oak/birch/maple droop ([#5](https://github.com/julien-riel/palubicki/issues/5), PR #60). | M | L | **DONE** — removes the "leaves stuck to stem" look; unblocks per-leaf orientation. |
...
| Simple vs compound leaf (pinnate / palmate / bipinnate) | ✅ | ⬆ Implemented | `geom/compound_leaf.py` — leaflets laid out in a pure-2D `compound_layout` on a rachis, rachis emitted as a separate stem-material primitive; gated on `leaf_kind != "simple"` (current species presets untouched). pinnée/palmée/bipinnée — PR #58. | H (ash, walnut, rose, sumac, mimosa, locust) | L | **DONE** ([#6](https://github.com/julien-riel/palubicki/issues/6), PR #58) — compound leaves now build end-to-end (leaf + rachis primitives, geometry grows linearly with leaflet count). |
...
| Leaf orientation (per-leaf insertion + petiole twist) | ✅ | ⬆ per-leaf | Each `Leaf` carries its own seating `azimuth` from `leaf_azimuths` (per-axis phyllotactic ordinal #24); the renderer reconstructs the blade basis per leaf from stem direction + azimuth + `leaf_splay_deg`. Leaves now spiral along the stem at true divergence instead of an even render-time fan ([#14](https://github.com/julien-riel/palubicki/issues/14)). | M | L | **DONE** — per-leaf insertion azimuth. Petiole *geometry* (a stalk) now landed ([#5](https://github.com/julien-riel/palubicki/issues/5), PR #60); per-leaf twist would build on this azimuth. |
| Leaf clusters at growing tips | ✅ | ⬆ Strengthened | `leaf_cluster_count`, `foliage_depth`, plus `needle_cluster_spacing` ([#36](https://github.com/julien-riel/palubicki/issues/36)) — clusters now distribute *along* each leaf-bearing internode (not just beaded at nodes), so conifer shoots are clothed over their length instead of tipped with green. | — | — | **DONE** — fixes the bare-branch-between-nodes look. |
| Needles / scales / fascicles | 🟡 | ⬆ Improved | Conifer crowns no longer read sparse: along-shoot needle distribution + larger needle footprint (`leaf_size`/`leaf_aspect`) recalibrated for pine & fir ([#36](https://github.com/julien-riel/palubicki/issues/36)) give full evergreen crowns (pine `total_leaf_area` 30→361; fir was near-bald). Still no real fascicles (clusters of 2–5 needles from a short shoot) — they're approximated by `leaf_cluster_count`. | M (close-up); L (distant) | L | **ADD** ([#7](https://github.com/julien-riel/palubicki/issues/7)) — `fascicle: int` parameter on the leaf primitive. Skip if all renders are tree-scale. |
...
| Leaf life span | 🟡 | ⬆ expressible | `Leaf.birth_time` (years) + `Leaf.age(clock)` now exist on every leaf ([#14](https://github.com/julien-riel/palubicki/issues/14)); `LeafState` enumerates `ACTIVE`/`SENESCENT`/`ABSCISSED`. The *age is recorded* but no time-based transition rule runs yet (all leaves stay `ACTIVE`). | M | L | **ADD** — the data is in place; wire the age→state rule (caducity follow-up on #14). |
| Deciduous / evergreen / marcescent | ❌ | ⬆ foundation ready | `birth_time` + `annual_growth_period` + `LeafState` all present; the abscission state machine is not written. | H (seasonal scenes) | L | **ADD** — build the `ACTIVE`→`SENESCENT`→`ABSCISSED` rule on the [#14](https://github.com/julien-riel/palubicki/issues/14) foundation; the renderer already filters on state, so dropping leaves is a state flip + golden re-pin. Payoff is enormous if seasonal renders matter. |

**Verdict.** Parametric blade + margins ([issue #4](https://github.com/julien-riel/palubicki/issues/4)), sun/shade morphology (Phase 2C), and now **leaves-on-nodes** ([#14](https://github.com/julien-riel/palubicki/issues/14)) are in: every leaf is a first-class `Leaf` with per-leaf age, state, and a true phyllotactic seating azimuth, and the renderer/diagnostics read it through one shared selector (leaf area held at exact parity). The render-time foliage-site synthesis is gone. Compound leaves ([#6](https://github.com/julien-riel/palubicki/issues/6), PR #58) now land too: pinnate/palmate/bipinnate leaflets on a rachis (separate stem-material primitive), gated on `leaf_kind != "simple"`. **Petiole geometry ([#5](https://github.com/julien-riel/palubicki/issues/5), PR #60) now lands** too: simple leaves seat the blade at the tip of a tapered, optionally-drooping `petiole`-material stalk (needles stay sessile). **Remaining: caducity/deciduousness (the age→state rule on the #14 foundation)** — best done jointly with seasonal cycles. Re-tune the species blade presets now that leaves seat at correct divergence (constat 4).
...
| Conifers (pine, fir) | ✅ (richer) | ⬆ Strengthened | Distichous needles on plagiotropic branches landed ([issue #15](https://github.com/julien-riel/palubicki/issues/15), `distichous_on_plagiotropic`); flat tiered sprays render correctly. Crowns now read full, not sparse/bald: along-shoot needle distribution + larger needle footprint ([#36](https://github.com/julien-riel/palubicki/issues/36)). Remaining: fascicles (clusters of 2–5 needles); cones (close-up only). | Add `fascicle` parameter for close-up correctness; cones when close-up conifer renders matter. |
...
1. **Shrub species presets** (§10) — now that `bud_break_bias` is in, lilac / dogwood / blueberry YAMLs are mostly parameter-tuning work. Visible payoff for a small PR.
2. ~~**Petiole geometry** (§6, [#5](https://github.com/julien-riel/palubicki/issues/5))~~ — ✅ landed (PR #60): tapered `petiole`-material stalk between bud site and blade, with gravity droop; needles sessile.
3. **Fascicles of needles** (§6, [#7](https://github.com/julien-riel/palubicki/issues/7)) — `fascicle: int` on the leaf primitive. Worth it only for close-up conifer renders.

*Landed since the previous review and removed from this list:* **compound leaves** ([#6](https://github.com/julien-riel/palubicki/issues/6), PR #58), true distichous phyllotaxis ([#15](https://github.com/julien-riel/palubicki/issues/15)), root flare at trunk base ([#8](https://github.com/julien-riel/palubicki/issues/8)), diagnostic/validation harness ([#13](https://github.com/julien-riel/palubicki/issues/13)), bark variation by `Internode.diameter` ([#9](https://github.com/julien-riel/palubicki/issues/9)), time/phenology foundation ([#10](https://github.com/julien-riel/palubicki/issues/10)).

*Newly unblocked by leaves-on-nodes ([#14](https://github.com/julien-riel/palubicki/issues/14)):* **caducity / deciduousness** — the age→`LeafState` rule (the renderer already filters on state, so it's a state flip + golden re-pin), and per-leaf phototropism/herbivory all now have a `Leaf` to attach to.
