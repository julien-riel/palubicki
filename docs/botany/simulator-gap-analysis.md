...
**Verdict.** This section is now complete for the species in scope. The cross-cutting per-axis delivery defect is **fixed** ([#24](https://github.com/julien-riel/palubicki/issues/24)): divergence advances per-axis via `Bud.axis_node_ordinal`, so spiral, decussate and distichous are all delivered correctly along each axis on the real tree (guarded by [#25](https://github.com/julien-riel/palubicki/issues/25)). With true distichous ([issue #15](https://github.com/julien-riel/palubicki/issues/15)) + `distichous_on_plagiotropic` giving flat 2-ranked conifer sprays, only the juvenile-vs-adult phyllotaxis switch remains (**SKIP**).
