...
systèmes.**
1. **#5, #7 — foliage (suite)** · #14 (feuilles first-class sur `Node`, `Leaf`/`LeafState`) **a atterri** et débloque la suite : pétiole (#5), fascicules d'aiguilles (#7) (feuilles composées #6 livrées, PR #58). La caducité / couleur d'automne (état `SENESCENT`/`ABSCISSED` posé mais non câblé) suit aussi sur cette fondation. Note : #7 (fascicules) raffinera les aiguilles de conifères posées par #36 ; revisiter les presets de lame d'espèce maintenant que les feuilles sont assises à la divergence phyllotactique (constat 4).
...
| #6 | Feuilles composées (pinnée/palmée/bipinnée) : leaflets sur rachis, layout 2D pur (`compound_layout`), rachis en primitive stem-material séparée, gating `leaf_kind != "simple"` (presets actuels intouchés) | #58 |
