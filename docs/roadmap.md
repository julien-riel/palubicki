...
1. #10 — fondation temporelle (Clock + `birth_time`). À ce stade tu as ressenti où le système actuel coince et tu sais ce que `LeafState` doit contenir.
2. #20 — longueur d'internode pilotée par la vigueur (flux Borchert-Honda continu ; retire `age_factor` + le cap `n_substeps_max`). Rend les quantités de croissance émergentes plutôt qu'imposées. S'appuie sur #10 pour démêler le proxy itération-comme-temps.
3. #14 — promouvoir les feuilles en attribut de `Node` (`Leaf` + `LeafState`). Refactor architectural, débloque tout ce qui suit côté foliage.
4. #6 puis #5, #7 — la suite foliage, construite sur #10 + #14 (donc `LeafState` conçu une seule fois).
5. #11, #12 — beaucoup plus tard.
